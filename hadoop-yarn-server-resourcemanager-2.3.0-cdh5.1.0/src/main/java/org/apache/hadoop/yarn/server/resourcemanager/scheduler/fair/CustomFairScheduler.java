...
public class CustomFairScheduler extends FairScheduler {
  ...
  private static final Log LOG = LogFactory.getLog(CustomFairScheduler.class);
  ...
  private long DEFAULT_BLOCK_SIZE = 67108864;
  //private long DEFAULT_BLOCK_SIZE = 1024;
  ...
  private static volatile HashMap<String,HashSet<String>> blockLocations = new HashMap<String,HashSet<String>>();
  ...
  /**
   * A runnable which calls {@link CustomFairScheduler#update()} every
	  ...
	  Set<NodeId> blackListNodes = new HashSet<NodeId>(nodes.size()); //This set contains the nodes that are already have a copy of the current replica
  	  ...
  	  while (targetNode == null) {
  		  ...
  		  if (!blockLocations.get(tp.getBlockId()).contains(targetNode.getNodeID().getHost())) {
  			  ...
  			  /*allocationToAM.addNewBlock(tp.getBlockId(), targetNode.getNodeID().getHost());*/
  			  System.out.println("Copying Block " + tp.getBlockId() + " To " + targetNode.getNodeID().getHost() + " totalNumberOfCopies " + ++totalNumberOfCopies );
  			  boolean isReplicaSuccessfull = replicateToPhysicalMachine(tp.getBlockId() , new String[]{targetNode.getNodeID().getHost()});
  			  if (isReplicaSuccessfull) {
  				  System.out.println("Replicated Successfully ");
  			  }
  			  else {
...
private boolean replicateToPhysicalMachine(String blockId, String []targetMachines) {
	//blockId example: hdfs://localhost:35400/user/yehia/Readme/setReplicationCallPath.txt_0
	try 
	{
		
		fs = FileSystem.get(conf);
		final URI nnURI = new URI(conf.get("fs.defaultFS"));
		
		String []blockIdStartingBytes = blockId.split("_");
		String relativeFilePath = "/u"+blockIdStartingBytes[0].split("/u")[1];
		final Path filePath = new Path(relativeFilePath);
		final long startingByte = Long.parseLong(blockIdStartingBytes[1]);
		System.out.println("UserGroupInformation.getLoginUser() " + UserGroupInformation.getLoginUser());
		UserGroupInformation ugi = UserGroupInformation.createRemoteUser(UserGroupInformation.getCurrentUser().getUserName());
		ugi.doAs(new PrivilegedExceptionAction<Void>() {
			@Override
			public Void run() throws Exception {
				dfsClient = new DFSClient(nnURI, conf);
				return null;
			}
		});
		
		List<LocatedBlock> locatedBlocks = dfsClient.getNamenode().getBlockLocations(filePath.toUri().toString(), startingByte,DEFAULT_BLOCK_SIZE ).getLocatedBlocks();
		LocatedBlock block = locatedBlocks.get(0);
		List<String> targetHosts = Arrays.asList(targetMachines);
		BlockRepInfo blockInfo = BlockRepInfo.newBuilder().setBlockId(block.getBlock().getBlockId()+"")
		          							 .addAllTargetHosts(targetHosts)
		          							 .build();
		FileStatus fileStatus = fs.getFileStatus(filePath);
		short numberOfReplicas = (short)fs.getFileBlockLocations(fileStatus, startingByte, startingByte+ DEFAULT_BLOCK_SIZE)[0].getHosts().length;
		if (++numberOfReplicas <= MAX_BLOCK_BUDGET) {
			String[] blackListedHosts = fs.getFileBlockLocations(fileStatus, startingByte, startingByte + DEFAULT_BLOCK_SIZE)[0].getHosts();
			for (String targetHost : targetHosts) {
				if (!Arrays.toString(blackListedHosts).contains(targetHost)) {
					dfsClient.setReplication(filePath.toUri().toString(), blockInfo, numberOfReplicas);
					Thread.sleep(3000);
				}
			}
		}
...
public synchronized void updateGain(int gain) {
	totalGain += gain;
	System.out.println("Current gain " + gain );
}
...
public void initNetworkTopology(HashMap<String, String> topology , String []topologyInfo) {
	if (topologyInfo == null) { //to pass TestReplicaSimulation Test case
		  ...
		  topology.put("slave9", "rack1");
		  topology.put("slave13", "rack1");
		  
		  topology.put("slave2", "rack2");
		  topology.put("slave6", "rack2");
		  topology.put("slave10", "rack2");
		  topology.put("slave14", "rack2");
		  
		  topology.put("slave3",  "rack3");
		  topology.put("slave7", "rack3");
		  topology.put("slave11", "rack3");
		  topology.put("slave15", "rack3");
		  
		  topology.put("slave4", "rack4");
		  topology.put("slave8", "rack4");
		  topology.put("slave12", "rack4");
		  topology.put("slave16", "rack4");
		
		  /*topology.put("slave1.novalocal", "rack1");
		  topology.put("slave2.novalocal", "rack1");
		  
		  topology.put("slave3.novalocal", "rack2");
		  topology.put("slave4.novalocal", "rack2");
		  
		  topology.put("slave5.novalocal",  "rack3");
		  topology.put("slave6.novalocal", "rack3");
		  
		  topology.put("slave7.novalocal", "rack4");
		  topology.put("slave8.novalocal", "rack4");*/
		}
	else {
  ...
  public void reinitialize(Configuration conf, RMContext rmContext)
    ...
    if (!initialized) {
      synchronized (this) {
	      ...
	      SCORE_THRESHOLD = this.conf.getDouble(SCORE_THRESHOLD_KEY, 5.2d);
	      nodeSelectionPolicy =  this.conf.getEnum(NODE_SELECTION_KEY, NodeSelection.RANDOM);
	      
	      //conf.set("hadoop.job.ugi", "yehia");
	      //System.setProperty("HADOOP_USER_NAME" , "yehia");
	      fs = FileSystem.get(conf);
  ...
  private class AllocationReloadListener implements
    ...
    public void onReload(AllocationConfiguration queueInfo) {
      ...
      synchronized (CustomFairScheduler.this) {
        allocConf = queueInfo;
